namespace QueryBuilder.Mongo.Extensions
{
    ...
    {
        public static IFindFluent<T, T> Build<T>(this IMongoCollection<T> collection, IRequest request)
            where T : class
        {
        ...
        public static IFindFluent<T, T> Build<T>(this IRequest request, IMongoCollection<T> collection)
            where T : class
        {
            return new MongoQueryBuilder<T>(request).Query(collection);
        }
